...
def minimax_decision(state, game):
    ...
    return argmax(game.actions(state),
                  key=lambda a: min_value(game.result(state, a))), getCount()
...
def alphabeta_search(state, game):
    ...
    def max_value(state, alpha, beta, count):
        ...
        v = -infinity
        for a in game.actions(state):
            tmp, count = min_value(game.result(state, a), alpha, beta, count)
            v = max(v, tmp)
            if v >= beta:
                return v, count
            alpha = max(alpha, v)
    ...
    def min_value(state, alpha, beta, count):
        ...
        v = infinity
        for a in game.actions(state):
            tmp, count = max_value(game.result(state, a), alpha, beta, count)
            v = min(v, tmp)
            if v <= alpha:
                return v, count
            beta = min(beta, v)
    ...
    best_action = None
    for a in game.actions(state):
        v, count = min_value(game.result(state, a), best_score, beta, count)
        if v > best_score:
            best_score = v
            best_action = a
...
class Game:
    ...
    def result(self, state, move):
        ...
        state.board[move] = state.to_move #add the move to the board
        state.moves.remove(move) #remove the move from the move list
        if state.to_move == 'X': #update whose turn is it after the move.
            state.to_move = 'O'
        else:
            state.to_move = 'X'
        return state
